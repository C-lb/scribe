...
Expected: prints `9`. Verify `.env` is gitignored with `git check-ignore .env` — it must print `.env`.
  ...
  private async processChunk(input: {
    ...
    try {
      if (this.config.keepAudio) {
        const name = `${String(input.index).padStart(4, "0")}.wav`;
        await writeFile(path.join(this.dir, "audio", name), input.audio);
      }

      this.audioSeconds += (input.endMs - input.startMs) / 1000;
      ...
      try {
        ...
        line = text
          ? this.transcript.record({ ...input, text })
          : this.transcript.recordFailure(input);
      } catch (error) {
        console.error(`[scribe] chunk ${input.index} transcription failed:`, error);
        this.failedChunks += 1;
        line = this.transcript.recordFailure(input);
      }
      ...
      console.error(`[scribe] chunk ${input.index} handling failed:`, error);
    }
  }
...
- Produces:
  ...
  - `createUploader({ sessionId, maxQueueSeconds, onStatus }): { enqueue(chunk): void, drain(): Promise<void> }`.
...
Run: `cd ~/scribe && npm test`
Expected: every unit test passes; both live suites skipped.
